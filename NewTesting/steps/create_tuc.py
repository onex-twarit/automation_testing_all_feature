...
def step_impl(context, ta_username, ta_invalid_pass):
    ta_username = context.test_data['TUC'][ta_username]
    ta_invalid_pass = context.test_data['TUC'][ta_invalid_pass]
    time.sleep(0.5)
    Login(context.driver).enter_txt_field(Login.input_field_login, ta_username)

    time.sleep(0.5)
    Login(context.driver).enter_txt_field(Login.input_field_password, ta_invalid_pass)


@when('Click on login button then enter valid password "{ta_password}"')
def step_impl(context, ta_password):
...
def step_impl(context, tuc_name, validity):
    ...
    if checkbox_default_gateway:
        Login(context.driver).click_button(Tuc.onex_gateway)
...
def step_impl(context):
    time.sleep(0.5)
    Login(context.driver).click_button(Credits.credits_left_menu)
