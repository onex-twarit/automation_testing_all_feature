...
@then('go to entity ID tab, click on add entitity ID')
...
@then('hindi, fill all the fields, then click on add')
def add_template_data(context):
    # click on add template
    time.sleep(2)
    button = context.driver.find_element(By.ID, 'create_template')
    ...
    # enter template name
    time.sleep(1)
    context.driver.find_element(By.ID, 'template_name').send_keys("Hindi Short")

    # enter template ID
    time.sleep(1)
    context.driver.find_element(By.ID, 'template_id').send_keys(12345678)
    ...
    context.driver.find_element(By.XPATH, '//*[@id="template_content"]').send_keys(
        "इस लेख में हम आपको 9वीं कक्षा के लेखन कौशल के विषय ‘सन्देश लेखन’ के बारे में बता रहे हैं।")
