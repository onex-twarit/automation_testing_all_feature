...
@then('add campaign \'quick split schedule\', fill the present fields, click on Add')
def step_impl(context):
    ...
    context.driver.find_element(By.XPATH,
        ...
        "quick split schedule")  # campaign name
    time.sleep(0.5)
...
@then('verify with name \'quick split schedule\'')
def step_impl(context):
    time.sleep(2)
    ...
    if text == 'quick split schedule':
        assert True, "Test Passed"
    else:
        assert False, "----->> Failed"


@then('go to scheduled campaign, verify \'quick split schedule\' name')
def step_impl(context):
    time.sleep(2)
