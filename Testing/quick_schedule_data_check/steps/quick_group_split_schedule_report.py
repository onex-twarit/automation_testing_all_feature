...
@then('add campaign \'split group camp\', fill the present fields, click on Add')
def step_impl(context):
    ...
    context.driver.find_element(By.XPATH,
        ...
        "split group camp")  # campaign name
    time.sleep(0.5)
...
@then('verify with name \'split group camp\'')
def step_impl(context):
    time.sleep(2)
    ...
    if text == 'split group camp':
        assert True, "Test Passed"
    else:
        assert False, "----->> Failed"


@then('go to scheduled campaign, verify \'split group camp\' name')
def step_impl(context):
    time.sleep(2)
