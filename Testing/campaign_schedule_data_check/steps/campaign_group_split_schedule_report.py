...
@then('add campaign \'campaign group split schedule\', fill the present fields, click on Add')
def step_impl(context):
    ...
    context.driver.find_element(By.XPATH,
        ...
        "campaign group split schedule")  # campaign name
    time.sleep(0.5)
...
@then('search with campaign name \'campaign group split schedule\'')
def step_impl(context):
    time.sleep(1)
    context.driver.find_element(By.ID, 'web_camp').send_keys(
        'campaign group split schedule')  # search with campaign name
...
@then('verify with name \'campaign group split schedule\'')
def step_impl(context):
    time.sleep(1)
    ...
    if text == 'campaign group split schedule':
        assert True, "Test Passed"
    else:
        assert False, "----->> Failed"


@then('go to scheduled campaign, verify \'campaign group split schedule\' name')
def step_impl(context):
    time.sleep(0.5)
