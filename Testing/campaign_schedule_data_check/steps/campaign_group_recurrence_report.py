...
@then('add campaign \'campaign group recurrence\', fill the present fields, click on Add')
def step_impl(context):
    ...
    context.driver.find_element(By.XPATH,
        ...
        "campaign group recurrence")  # campaign name
    time.sleep(0.5)
...
@then('search with campaign name \'campaign group recurrence\'')
def step_impl(context):
    time.sleep(1)
    context.driver.find_element(By.ID, 'web_camp').send_keys('campaign group recurrence')  # search with campaign name
...
@then('verify with name \'campaign group recurrence\'')
def step_impl(context):
    time.sleep(1)
    ...
    if text == 'campaign group recurrence':
        assert True, "Test Passed"
    else:
        assert False, "----->> Failed"


@then('go to scheduled campaign, verify \'campaign group recurrence\' name')
def step_impl(context):
    time.sleep(0.5)
