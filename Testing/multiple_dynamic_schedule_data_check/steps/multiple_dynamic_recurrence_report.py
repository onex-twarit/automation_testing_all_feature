...
@then('add campaign \'multiple dynamic recurrence\', fill the present fields, click on Add')
def step_impl(context):
    ...
    context.driver.find_element(By.XPATH,
        ...
        "multiple dynamic recurrence")  # campaign name
    time.sleep(1)
...
@then('search with campaign name \'multiple dynamic recurrence\'')
def step_impl(context):
    time.sleep(1)
    context.driver.find_element(By.ID, 'web_camp').send_keys(
        'multiple dynamic recurrence')  # search with campaign name
...
@then('verify with name \'multiple dynamic recurrence\'')
def step_impl(context):
    time.sleep(1)
    ...
    if text == 'multiple dynamic recurrence':
        assert True, "Test Passed"
    else:
        assert False, "----->> Failed"


@then('go to scheduled campaign, verify \'multiple dynamic recurrence\' name')
def step_impl(context):
    time.sleep(0.5)
