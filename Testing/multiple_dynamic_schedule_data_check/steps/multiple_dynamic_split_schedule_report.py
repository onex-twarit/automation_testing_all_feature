...
@then('add campaign \'multiple dynamic split schedule\', fill the present fields, click on Add')
def step_impl(context):
    ...
    context.driver.find_element(By.XPATH,
        ...
        "multiple dynamic split schedule")  # campaign name
    time.sleep(1)
...
@then('search with campaign name \'multiple dynamic split schedule\'')
def step_impl(context):
    time.sleep(1)
    context.driver.find_element(By.ID, 'web_camp').send_keys(
        'multiple dynamic split schedule')  # search with campaign name
...
@then('verify with name \'multiple dynamic split schedule\'')
def step_impl(context):
    time.sleep(1)
    ...
    if text == 'multiple dynamic split schedule':
        assert True, "Test Passed"
    else:
        assert False, "----->> Failed"


@then('go to scheduled campaign, verify \'multiple dynamic split schedule\' name')
def step_impl(context):
    time.sleep(0.5)
