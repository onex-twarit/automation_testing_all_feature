...
@then('add campaign \'unicode recurrence\', fill the present fields, click on Add')
def step_impl(context):
    ...
    context.driver.find_element(By.XPATH,
        ...
        "unicode recurrence")  # campaign name
    time.sleep(0.5)
...
@then('verify with name \'unicode recurrence\'')
def step_impl(context):
    time.sleep(2)
    ...
    if text == 'unicode recurrence':
        assert True, "Test Passed"
    else:
        assert False, "----->> Failed"


@then('go to scheduled campaign, verify \'unicode recurrence\' name')
def step_impl(context):
    time.sleep(2)
