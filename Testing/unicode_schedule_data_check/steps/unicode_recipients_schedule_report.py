...
@then('click on new sms, click on unicode sms')
def step_impl(context):
    time.sleep(0.5)
    context.driver.find_element(By.XPATH, '//*[@id="new_sms"]').click()  # go to NEW SMS

    time.sleep(0.5)
    button = context.driver.find_element(By.XPATH, '//*[@id="uicode_sms_title"]')
    context.driver.execute_script("arguments[0].click();", button)  # go to unicode SMS


@then('add campaign \'unicode camp\', fill the present fields, click on Add')
def step_impl(context):
    time.sleep(0.5)
...
@then('check allow multi part and allow unicode')
def step_impl(context):
    time.sleep(1)
...
@then('verify with name \'unicode camp\'')
def step_impl(context):
    time.sleep(2)
...
@then('go to scheduled campaign, verify \'unicode camp\' name')
def step_impl(context):
    time.sleep(2)
