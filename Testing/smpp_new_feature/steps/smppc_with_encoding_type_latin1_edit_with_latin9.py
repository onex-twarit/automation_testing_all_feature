...
@then('fill the details to create SMPPC with LATIN-1 encoding type')
def step_impl(context):
    # enter TUC name
    time.sleep(0.5)
    context.driver.find_element(By.ID, 'name').send_keys("smppcTwo")
    ...
    # SMPP login ID
    time.sleep(0.5)
    context.driver.find_element(By.ID, 'smpplogin').send_keys("1111")

    # System ID
    time.sleep(0.5)
    context.driver.find_element(By.ID, 'username').send_keys("2222")
...
@then('click on edit SMPPC for LATIN-1 encoding type')
def step_impl(context):
    time.sleep(0.5)
    button = context.driver.find_element(By.XPATH,
                                         "//td[contains(text(),'smppcTwo')]/following::td[5]/a[@title='Edit']")
    context.driver.execute_script("arguments[0].click();", button)


@then('verify the SMPPC LATIN-1 encoding type')
def step_impl(context):
    time.sleep(1)
...
@then('SMPPC encoding type LATIN-1 to LATIN-9')
def step_impl(context):
    time.sleep(1)
    select = Select(context.driver.find_element(By.ID, 'encoding_type'))
...
@then('verify the updated SMPPC LATIN-9 encoding type')
def step_impl(context):
    time.sleep(1)
    text = context.driver.find_element(By.ID, 'encoding_type').text
