...
@then('add campaign \'dynamic recurrence camp\', fill the present fields, click on Add')
def step_impl(context):
    ...
    context.driver.find_element(By.XPATH,
        ...
        "dynamic recurrence camp")  # campaign name
    time.sleep(0.5)
...
@then('search with campaign name \'dynamic recurrence camp\'')
def step_impl(context):
    time.sleep(1)
    context.driver.find_element(By.ID, 'web_camp').send_keys('dynamic recurrence camp')  # search with campaign name
...
@then('verify with name \'dynamic recurrence camp\'')
def step_impl(context):
    time.sleep(1)
    ...
    if text == 'dynamic recurrence camp':
        assert True, "Test Passed"
    else:
        assert False, "----->> Failed"


@then('go to scheduled campaign, verify \'dynamic recurrence camp\' name')
def step_impl(context):
    time.sleep(0.5)
